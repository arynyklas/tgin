...
- **Constant-time secret compares go through `utils::auth::secret_eq`** — do not hand-roll another `ct_eq` dance. The optional control-plane `Bearer` gate is `utils::auth::guard`; an empty `auth_token` is a config validation error (`ValidationError::EmptyAuthToken`).
