...
| `/api/route`  | `POST`   | `{ "type": "Webhook"\|"Longpull", ..., "sublevel": 0 }` | Adds a route dynamically. `Webhook` form: `{"type": "Webhook", "url": "...", "request_timeout_ms": <ms>?}`. `Longpull` form: `{"type": "Longpull", "path": "..."}`. `request_timeout_ms` defaults to `10000`. `sublevel` is reserved for hierarchical insertion (currently a placeholder). |
